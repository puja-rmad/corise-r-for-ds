...
tbl_names_popular |> 
  ...
  labs(
    title = 'Top 5 Most Popular US Names of the 2010s',
    subtitle = 'Emma and Noah are the most popular names of the decade (2011-2020)',
    caption = 'Source: SSA',
    x = 'Number of Births',
    y = NULL
  ) +
...
tbl_names_popular_trendy = tbl_names |> 
  ...
  mutate(trendy = nb_births_max / nb_births_total) |> 
  # Group by sex
  group_by(sex) |> 
  # Slice top 5 rows by trendiness for each group
  slice_max(trendy, n = 5)
...
```
    ...
    #> # A tibble: 10 × 5
    #> # Groups:   sex [2]
    #>    sex   name      nb_births_total nb_births_max trendy
    #>    <chr> <chr>               <dbl>         <dbl>  <dbl>
...
tbl_names_by_letter |> 
  ...
  labs(
    title = "US Babynames' First Letter Distribution for the year 2020",
...
plot_trends_in_letter <- function(my_letter) {
  tbl_names_by_letter |> 
    ...
    # Add labels (title, subtitle, caption, x, y)
    labs(
      title = glue::glue("Trends in Babynames beginning with {my_letter}"),
      caption = "Source: SSA",
      x = "Year",
      y = '% of Births'
    ) +
...
tbl_names_by_first_and_last_letter = tbl_names |> 
  ...
  summarise(nb_births = sum(nb_births)) |>
...
tbl_names_by_first_and_last_letter |> 
 ...
 labs(
   title = "Distribution of births by first and last letter of name for the 2021",
